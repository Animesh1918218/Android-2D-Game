...
public class GameEngine {
    ...
    public GameEngine() {
        ...
        for (int i = 0; i < AppConstants.numberOfTubes; i++) {
            int tubeX = AppConstants.screenWidth + i * AppConstants.distanceBetweenTubes;
    ...
    public void updateAndDrawTubes(Canvas canvas) {
        if (gameState == 1) {
            if ((tubes.get(scoringTube).getTubeX() < bird.getX() + AppConstants.getBitmapBank().getBirdWidth()) && (tubes.get(scoringTube).getTopTubeOffsetY() > bird.getY() || tubes.get(scoringTube).getBottomTubeY() < ( bird.getY() + AppConstants.getBitmapBank().getBirdHeight()))) {
                gameState = 2;
                //Log.d("Game", "Over");
                AppConstants.getSoundBank().playHit();
                AppConstants.getSoundBank().stop();
            ...
            for (int i = 0; i < AppConstants.numberOfTubes; i++) {
                ...
                tubes.get(i).setTubeX(tubes.get(i).getTubeX() - AppConstants.tubeVelocity);
                 if (tubes.get(i).getTubeColor() == 0) {
                    ...
                    {
                        canvas.drawBitmap(AppConstants.getBitmapBank().getSkyTubeTop(), tubes.get(i).getTubeX(), tubes.get(i).getTopTubeY(), null);
                        canvas.drawBitmap(AppConstants.getBitmapBank().getSkyTubeBottom(), tubes.get(i).getTubeX(), tubes.get(i).getBottomTubeY(), null);
                    }
                } else {
                    canvas.drawBitmap(AppConstants.getBitmapBank().getPinkTubeTop(), tubes.get(i).getTubeX(), tubes.get(i).getTopTubeY(), null);
    ...
    public void updateAndDrawBackgroundImage(Canvas canvas) {
        ...
        if (backgroundImage.getX() < -(AppConstants.getBitmapBank().getBackgroundWidth() - AppConstants.screenWidth)) {
    ...
    public void updateAndDrawBird(Canvas canvas) {
        if (gameState == 1) {
            if (bird.getY() < (AppConstants.screenHeight - AppConstants.getBitmapBank().getBirdHeight()) || bird.getVelocity() < 0) {
